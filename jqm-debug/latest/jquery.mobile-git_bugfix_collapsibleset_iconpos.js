...
$.widget( "mobile.collapsible", {
	...
	_setOptions: function( options ) {
		...
		if ( opts.iconpos !== undefined ) {
			if ( ui.accordion.length ) {
				currentOpts.iconpos = this._getCurrentIconpos( anchor );
			}
	...
	_getCurrentIconpos: function( anchor ) {
		var iconpos = anchor.attr( "class" ).match( /ui-btn-icon-[a-z]+/g );
		if ( !iconpos.length ) {
			return;
		}

		iconpos = iconpos[ 0 ].match( /-[a-z]+/g );
		iconpos = iconpos[ iconpos.length - 1 ].substring( 1 );
