...
$.widget( "mobile.collapsible", {
	...
	_create: function() {
		var elem = this.element,
			ui = {
				// TODO: ":jqmData(role='collapsible-set')": Deprecated in 1.4, remove in 1.5. */
				accordion: elem
					.closest( ":jqmData(role='collapsibleset'), :jqmData(role='collapsible-set')" +
		...
		if ( isCollapsed ) {
			...
			if ( opts.collapsedIcon !== undefined ) {
				if ( ui.accordion.length ) {
					currentOpts.collapsedIcon = this._getCurrentIcon( anchor );
				}
			...
			if ( opts.expandedIcon !== undefined ) {
				if ( ui.accordion.length ) {
					currentOpts.expandedIcon = this._getCurrentIcon( anchor );
				}
	...
	_getCurrentIcon: function( anchor ) {
		var icon = anchor.attr( "class" ).match( /ui-icon-[a-z]+/g );
		if ( !icon.length ) {
			return;
		}

		icon = icon[ 0 ].match( /-[a-z]+/g );
		icon = icon[ icon.length - 1 ].substring( 1 );
